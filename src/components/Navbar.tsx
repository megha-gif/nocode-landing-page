...
const Navbar = () => {
  ...
  return (
    // Navbar
    <div className=" bg-[linear-gradient(to_bottom,#000514_30%,#000A1F_100%)] min-h-screen w-full">
      <nav className="backdrop-blur-lg top-0 left-0 z-50 w-full bg-[linear-gradient(to_bottom,rgba(0,10,31,0.08)_30%,rgba(5,48,132,0.08)_100%)]">
        {/* Wrapping Div */}
        <div className="max-w-7xl flex items-center justify-between px-3 py-6 mx-auto text-white">
          ...
          >
            {menu ? (
              <Image src={menuclose} alt="Menu" width={40} height={40} />
            ) : (
          ...
          {/* Mobile Menu */}
          {menu && (
            <div className="pt-5 pb-5 font-inder top-full absolute  md:hidden left-0 flex flex-col items-center justify-center w-full mx-auto space-y-5 ">
              <Link href="#">
                <button
                  onClick={closeMenu}
              ...
              <Link href="/Goback">
                <button
                  onClick={closeMenu}
                  className="text-[19px] text-white p-5 px-[110px] bg-[#007ACC] rounded-[2px] hover:scale-105 font-montserrat"
                >
                  Get Started
                </button>
              </Link>
